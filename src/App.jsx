...
export default function App() {
  ...
  const router = createBrowserRouter(
    createRoutesFromElements(
      <Route
        element={<Sidebar movieList={movieList} setMovieList={setMovieList} />}
        errorElement={<ErrorPage />}
      >
        <Route errorElement={<ErrorPage />}>
          <Route
            path="/Movie-Tracker/movies/:movieId"
            element={<DisplayMoviePage />}
          />
          <Route
            path="/Movie-Tracker/"
            element={
              <DisplayList movieList={movieList} setMovieList={setMovieList} />
            }
          />
        </Route>
      </Route>,
...
function sidebar_buttons(
  ...
  const random_placeholder =
    random_movie[Math.floor(Math.random() * random_movie.length)];
          ...
          return (
            ...
            >
              ...
              <div>
                [
                {movie.runtime_hm ||
                  tmdbList.find((m) => m.id === movie.dbid).runtime_hm}
                ]
              </div>
              <div>
                (
...
function animate_grow_on_mouse_hover(
  mouseY,
  tbody,
  rowHeight,
  targetZoomedLineHeight,
  ...
  let prevRow3LineHeight = standardLineHeight - grow_size * ratioWithinRowInv;
  ...
  let nextRow3LineHeight = standardLineHeight - grow_size * ratioWithinRow;
...
export function DisplayMovie({ movie, idx, tmdb }) {
  ...
  return (
    <>
      <Link
        to={`/Movie-Tracker/movies/${moviePrev.id}`}
      ...
      <Link
        to={`/Movie-Tracker/movies/${movieNext.id}`}
...
async function get_details(movie, type) {
  ...
  if (type == "ratings") {
    url = new URL(
      `https://api.themoviedb.org/3/movie/${movie.dbid}/release_dates`,
    );
  } else {
  ...
  return (
    <div className="movie-top-bar">
      <div>
        ...
        <div className="movie-rating">{USrating?.rating}</div>
      </div>

      <div className="movie-title-block">
        <div>
          <h1
            className="movie-title-only"
            // style={{"--font_size": `${fontSize}px`}}
          >
            {movie.title}
          </h1>
          <h2>({movie.year || tmdb?.release_date?.slice(0, 4)})</h2>
          <h2>{movie.runtime_hm || tmdb.runtime_hm}</h2>
        </div>
      </div>
  ...
  return (
    <div className={`${css}`}>
      {Object.entries(movie.links).map((type_arr) => {
        ...
        return <YTlink key={link_type} type={link_type} url={link_urls} />;
      })}
    </div>
  );
}
